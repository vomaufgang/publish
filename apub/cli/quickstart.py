...
def main():
    ...
    while book_title is None or book_title.isspace():
        book_title = input('Every book must have a title, please enter the book\'s title: ')

    book_subtitle = input('Enter the book\' subtitle (optional, leave empty to skip): ')
    if book_subtitle is None or book_subtitle.isspace():
        book_subtitle = 'None'
    else:
        book_subtitle = "'{book_subtitle}'".format(book_subtitle=book_subtitle)

    authors = input('Enter the author\'s name (separate multiple authors by comma): ')
    first_chapter_name = input('Enter the name of the first chapter: ')
    first_chapter_id = input('Enter the id of the first chapter (may contain a-zA-Z0-9-_): ')
